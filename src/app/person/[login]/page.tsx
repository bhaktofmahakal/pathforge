...
export default function PersonPage() {
  ...
  return (
    <div className="container-narrow">
      ...
      <Link
        href="/"
        className="inline-flex items-center gap-2 text-xs sm:text-sm text-slate-400 hover:text-white mb-6 sm:mb-8 transition-colors font-mono"
      >
      ...
      >
        {(person) => (
          <div className="space-y-8">
            {/* Header card */}
            <div className="card-cognodb p-6 sm:p-8 flex flex-col md:flex-row items-center md:items-start gap-6 justify-between text-center md:text-left">
              <div className="flex flex-col sm:flex-row items-center sm:items-start gap-4 sm:gap-6">
                <img
                  src={person.avatarUrl}
                  alt={person.name}
                  className="w-20 h-20 sm:w-24 sm:h-24 rounded-full border-2 border-white/20 object-cover flex-shrink-0"
                />
                <div className="min-w-0">
                  <h1 className="text-2xl sm:text-4xl font-black text-white mb-1 truncate">
                    {person.name}
                  </h1>
                  <div className="flex flex-wrap items-center justify-center sm:justify-start gap-2 text-xs sm:text-sm text-slate-400 font-mono">
                    <span className="text-[#00f2fe] font-bold">@{person.login}</span>
                  ...
                  <div className="mt-4 flex items-center justify-center sm:justify-start gap-3">
                    <a
                      ...
                      className="inline-flex items-center gap-1.5 text-xs text-[#00f2fe] hover:underline font-mono transition-colors"
                    >
              ...
              <div className="grid grid-cols-1 sm:flex sm:flex-wrap gap-2.5 w-full md:w-auto">
                <Link href={`/path?from=${person.login}`} className="btn-mint text-xs">
                  Find Path
                </Link>
                <Link href={`/connect?me=${person.login}`} className="btn-dark-secondary text-xs">
                  Get Introduced
                </Link>
                <Link href={`/recommend?login=${person.login}`} className="btn-dark-secondary text-xs">
                  Recommended Repos
                </Link>
              </div>
            </div>
            ...
            {person.topCollaborators && person.topCollaborators.length > 0 && (
              <div>
                <h2 className="text-base sm:text-lg font-bold text-white mb-4 flex items-center gap-2">
                  <span>Closest Network Collaborators</span>
                  <span className="text-xs font-mono text-slate-500">({person.topCollaborators.length})</span>
                </h2>
                ...
                <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-6 gap-3">
                  {person.topCollaborators.map((collab) => (
                    <Link
                      ...
                      className="card-cognodb p-3 text-center flex flex-col items-center justify-between group"
                    >
                      <img
                      ...
                      />
                      <div className="font-bold text-xs text-white group-hover:text-[#00f2fe] transition-colors truncate w-full">
                        {collab.name}
                      </div>
                      <div className="text-[10px] text-slate-400 font-mono truncate w-full mb-1">
                        @{collab.login}
                      </div>
                      <span className="badge-mint !text-[9px] !py-0.5">
            ...
            <div>
              <h2 className="text-base sm:text-lg font-bold text-white mb-4 flex items-center gap-2">
                ...
                <span className="badge-mint font-mono">
                  {person.contributedRepos.length}
                </span>
              </h2>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                {person.contributedRepos.map((repo) => (
                  <div key={repo.fullName} className="card-cognodb p-5 flex flex-col justify-between group">
                    <div>
                      <div className="flex items-center justify-between gap-2 mb-2">
                        <a
                          ...
                          target="_blank"
                          rel="noopener noreferrer"
                          className="font-bold text-white hover:text-[#00f2fe] transition-colors text-sm sm:text-base flex items-center gap-1.5 break-all"
                        >
                          {repo.fullName}
                    ...
                    <div className="flex items-center justify-between text-xs text-slate-500 border-t border-white/5 pt-3 mt-2">
                      ...
                      <span className="text-[#00f2fe] font-semibold font-mono">
